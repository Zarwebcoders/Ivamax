...
const AdminLayout = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 relative overflow-x-hidden flex">
                ...
                <div className={`h-16 flex items-center ${sidebarOpen ? 'justify-start' : 'justify-center'} relative border-b border-gray-300 mx-4`}>
                    <div className={`flex items-center gap-3 transition-all duration-300 ${sidebarOpen ? 'scale-100' : 'scale-90'}`}>
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ${sidebarOpen ? 'w-auto opacity-100' : 'w-0 opacity-0'}`}>
                            <h1 className="font-bold text-xl tracking-tight text-gray-900 leading-tight">
                                Admin<span className="text-golden-600">Panel</span>
                            </h1>
                        </div>
                    </div>
                </div>
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                `}
                            >
                                {active && <div className="absolute left-0 top-2 bottom-2 w-1 bg-golden-500 rounded-r-full"></div>}

                                <Icon size={22} className={`flex-shrink-0 transition-transform duration-300 ${active ? 'scale-110' : 'group-hover:scale-110'}`} />
            ...
            >
                {isMobile
                    ? (sidebarOpen ? <ChevronLeft size={20} /> : <Menu size={24} />)
            ...
            `}>
                ...
                `}>
                    <div>
                        ...
                        {!isMobile && <p className="text-sm text-gray-500">Manage your platform efficiently</p>}
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {/* Admin Badge */}
                        <div className="hidden md:flex items-center gap-3 px-5 py-2.5 bg-golden-50 border border-golden-200 rounded-xl shadow-sm">
                            <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                            <span className="text-sm font-bold text-golden-800">Administrator</span>
                        </div>
                    </div>
                </header>
